...
case class SingleS3 (value: Double)

object StorableTaskS3Test {
  ...
  class TaskDS(implicit val p: Peapod) extends StorableTask[Dataset[SingleS3]] {
    def generate = {
      import p.sqlCtx.implicits._
      p.sc.makeRDD( SingleS3(1.0) :: SingleS3(2.0) :: Nil).toDS()
    }
  }

  class TaskSerializable(implicit val p: Peapod) extends StorableTask[SingleS3] {
    def generate = {
      SingleS3(1.0)
    }
  }
...
class StorableTaskS3Test extends FunSuite {
  test("Delete Old Version") {
    val p1 = PeapodGeneratorS3.peapod()
  ...
  test("Recursive Version") {
    val p1 = PeapodGeneratorS3.peapod()
    val p2 = PeapodGeneratorS3.peapod()
    val t1 = new TaskB1()(p1)
  ...
  test("SUCCESS file") {
    implicit val p = PeapodGeneratorS3.peapod()
